...
const createPlacement = asyncHandler(async (req, res) => {
  ...
  if (!mobile && !email) throw new ApiError(400, "Provide either mobile or email to identify student");
  if (!companyId) throw new ApiError(400, "Company ID is required");

  const student = await Student.findOne({ $or: [{ mobile }, { email }] });
  if (!student) throw new ApiError(404, "Student not found");

  const company = await Company.findById(companyId);
  if (!company) throw new ApiError(404, "Company not found");

  const exists = await Placement.findOne({ studentId: student._id, companyId: company._id });
  ...
  return res.status(201).json(new ApiResponse(201, placement, "Placement created successfully"));
});
...
const getPlacements = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, placements, "Placements fetched successfully"));
});
...
const getPlacementById = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, placement, "Placement fetched successfully"));
});
...
const updatePlacement = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, placement, "Placement updated successfully"));
});
...
const softDeletePlacement = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, placement, "Placement soft deleted"));
});
...
const restorePlacement = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, placement, "Placement restored successfully"));
});
...
const deletePlacement = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, {}, "Placement permanently deleted"));
});

/* GET ONLY REVIEWS */
const getReviewsOnly = asyncHandler(async (req, res) => {
  const reviews = await Placement.find({ isDeleted: false }, {
    fullName: 1, companyName: 1, rating: 1, review: 1, studentImage: 1
  }).sort({ createdAt: -1 });
...
const getOnlyPlacements = asyncHandler(async (req, res) => {
  const data = await Placement.find({ isDeleted: false }, {
    fullName: 1, department: 1, companyName: 1, companyDesignation: 1, studentImage: 1, companyImage: 1
  }).sort({ createdAt: -1 });

  return res.status(200).json(new ApiResponse(200, data, "Placements fetched successfully"));
});
